 ...
 * ditatompel effect **/
DitatompelPopupEffect = Class.create();
DitatompelPopupEffect.prototype = {
...
var sitehostname = "2nd-heartbeat";
var workingDir = "/home/ditatompel";
var ditatompelConsoleWelcome = "<pre>         mm<br />"
	+ "      /^(  )^\\  <br />"
	+ "      \\,(..),/      <br />"
	+ "        V~~V    -DitatompeL</pre>"
	+ "Welcome to Virtual Konsole version 0.1<br />"
	+ "These shell commands are defined internally.<br />Type '<strong>help</strong>' to see available commands.<br />"
	+ "Use '<strong>info</strong>' to show this message.<br /><br />";
var ditahost = konsoleLoginAs + "@" + sitehostname + " ~" + workingDir + " $";
...
function debug(input, command) {
	...
	switch(command) {
		case 'info':
			output = b + ditatompelConsoleWelcome;
		break;
		case 'hostname':
		...
		case 'ls -l':
			...
			+ b + "drwxr-x---  7 ditatompel ditatompel     4096 2011-10-07 04:01 Desktop"
			+ b + "drwxr-x--- 19 ditatompel ditatompel     4096 2011-10-04 07:38 Documents"
			+ b + "drwxr-x---  9 ditatompel ditatompel     4096 2011-10-11 13:43 Downloads"
			+ b + "drwxr-xr-x 12 ditatompel ditatompel     4096 2011-10-01 00:44 Music"
			+ b + "drwxr-xr-x 95 ditatompel ditatompel     4096 2011-09-07 10:13 Public"
			+ b + "-rw-------  1 ditatompel ditatompel     1665 2011-10-01 09:45 sonia.txt"
			+ b + "drwxr-x---  7 ditatompel ditatompel     4096 2011-09-07 22:11 Videos</pre>";
			
		break;
		case 'help':
			output = b + "available commands :"
			+ b + s + "<strong>info</strong> : show Konsole information" + b
	...
	$('debug').innerHTML +=  ditahost + output + "<br />";
	// $('debug').innerHTML +=  ditahost + " " + input + "<br />" + output + "<br />";
...
function ditatompelVirtualConsole() {
	if (debugWindow != null) {
		hideDebug();
	}
	...
	if (debugWindow == null) {
		...
		debugWindow = new Window('debug_window', {className: 'dialog',width:500, height:400, left:4, minWidth:250, bottom:42, zIndex:1000, opacity:1, showEffect: Element.show, resizable: true, title: "Konsole"})
		cont = " <input id=\"debug_window_command\" type=\"textbox\">";
		debugWindow.getContent().innerHTML = "<style>#debug_window .dialog_content {background: url('images/bg_trans_70.png') repeat 0 0;} #debug_window_command {width:40%; height: 12px; background: transparent url('images/bg_trans_100.png') repeat 0 0; border:none; font-size:10px; color:#FFF;}</style> <div id='debug'>"+ditatompelConsoleWelcome+"</div>" + ditahost + cont;
...
WindowUtilities.getPageSize = function() {
	...
	var dockHeight = $('ditatompel_dock_bottom').getHeight();
...
Object.extend(Windows, {
	...
	minimize: function(id, event) {
		...
		if (win && win.visible) {
			...
			element.className = "ditatompel_minimized_window"; 
			element.style.display = "none";
			element.win = win;
			$('ditatompel_dock_bottom').appendChild(element);
...
Event.observe(document, "click", function(event) {   
	...
	var win = e.up(".dialog");
	var dock = e == $('dock') || e.up("#ditatompel_dock_bottom"); 
...
jQuery(document).ready(function($){
	...
	$.backstretch("images/fisherman.jpg", {speed: 400}); // this is backstretch background image.
	
	/*
	 * Other mouse hover and click event goes here
	 */
	$('#ditatompel_inline_top_menu ul ').css({display: "none"}); // Opera Fix
	$('#ditatompel_inline_top_menu li').hover(function(){
		$('ul.the_menu').hide();
	...
	//menu slider
	$('div#ditatompel_logo').click(function () {
